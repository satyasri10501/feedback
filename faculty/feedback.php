...
if($r==false)
{
echo "<h3 style='color:Red'>Not found any records! </h3>";
}
else
{
?>
	...
	while($row=mysqli_fetch_array($q))
		{
			...
			echo "<td>".$row[13]."</td>";
			echo "<td>".$row[14]."</td>";
			$row[15]=($row[3]+$row[4]+$row[5]+$row[6]+$row[7]+$row[8]+$row[9]+$row[10]+$row[11]+$row[12]+$row[13]+$row[14])/60*100;
			echo "<td>".$row[15]."</td>";
			$total=$total+$row[15];
			echo "<td>".$row[16]."</td>";
			echo "<td>".$row[17]."</td>";
		...
		$count=$i-1;
		$avg=$total/$count;
		echo "whole class percentage is:",$avg;
		?>
